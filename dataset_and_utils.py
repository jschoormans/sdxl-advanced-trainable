...
def generate_random_mask(image):
    # Create a mask with the same batch size and spatial dimensions as the image, but with a single channel
    mask = torch.zeros_like(image[:, :1, :, :])  # Assuming image shape is (B, C, H, W) and mask needs to be (B, 1, H, W)
    
    # Calculate holes for the mask
    holes = _get_cutout_holes(image.shape[2], image.shape[3])  # Use spatial dimensions H and W from the image
    
    for (x1, y1, x2, y2) in holes:
        mask[..., y1:y2, x1:x2] = 1.0  # Use broadcasting to fill the holes in the mask
    
    if random.uniform(0, 1) < 0.25:
        mask.fill_(1.0)
    
    # Apply the mask to the image
    masked_image = image * (1 - mask)  # Use 1-mask to invert mask values for masking
...
class PreprocessedDataset(Dataset):
    def __init__(
        ...
        if do_cache:
            ...
            for idx in range(len(self.data)):
                token, vae_latent, mask,  random_mask_latents, masked_image_latents = self._process(idx)
                self.vae_latents.append(vae_latent)
                self.tokens_tuple.append(token)
                self.masks.append(mask)
                self.random_mask_latents.append(random_mask_latents)
                self.masked_image_latents.append(masked_image_latents)
    ...
    def _process(
        ...
        gen_mask, gen_masked_image = generate_random_mask(image)

        print(self.size)  # 512

        print(gen_mask.shape)
        print(gen_mask)
        print(gen_masked_image.shape)
        print(gen_masked_image)
        
        # random_mask = np.zeros((self.size, self.size))
        # random_mask[:self.size//2,:] = 1

        # random_mask = torch.from_numpy(random_mask).unsqueeze(0).unsqueeze(0).to(image.device)

        random_mask_latents = torch.nn.functional.interpolate(
            gen_mask, size=(self.size // 8, self.size // 8))
        
        # mask the image with the mask 
        # TODO - the actual masking
        masked_image = gen_masked_image 
        masked_image_latents =        self.vae_encoder.encode(masked_image).latent_dist.sample()

        # I assume this must be
        if self.scale_vae_latents:
            masked_image_latents = masked_image_latents * self.vae_encoder.config.scaling_factor
        ...
        return (ti1.squeeze(), ti2.squeeze()), vae_latent.squeeze(), mask.squeeze(), random_mask_latents.squeeze(dim=0), masked_image_latents.squeeze()
    ...
    def atidx(
        ...
        if self.do_cache:
            return self.tokens_tuple[idx], self.vae_latents[idx], self.masks[idx], self.random_mask_latents[idx], self.masked_image_latents[idx]
        else:
    ...
    def __getitem__(
        ...
        token, vae_latent, mask, random_mask_latents,  masked_image_latents = self.atidx(idx)
        return token, vae_latent, mask, random_mask_latents,  masked_image_latents
